fun main() {
    ...
    val input = """Austrálie|dolar|1|AUD|15,741\n" +
    ...
    val line: List<String> = input.split("\\n")
    for (i in line.indices) {
        val values = line[i].split("|").toTypedArray()
        val value = values[2].toInt() / values[4].replace(",", ".").toFloat()
        println(values[3] + " - " + value)
    }
